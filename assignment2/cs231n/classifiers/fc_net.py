...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    a2, l2_cache = affine_forward(r1, W2, b2)
    ...
    regularization_loss = 0.5 * self.reg * (np.sum(W1*W1) + np.sum(W2*W2))
    loss = data_loss + regularization_loss
    
    d_r1, d_W2, d_b2 = affine_backward(d_scores, l2_cache)
    d_a1 = relu_backward(d_r1, r1_cache)
    d_X, d_W1, d_b1 = affine_backward(d_a1, l1_cache)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    self.params['W' + str(self.num_layers)] = np.random.normal(
        loc=0,
        scale=weight_scale,
        size=(hidden_dims[-1], num_classes)).reshape(hidden_dims[-1], num_classes)
  ...
  def loss(self, X, y=None):
    ...
    for i in range(1, self.num_layers):
        Wi = self.params['W'+str(i)]
        regularization_loss += 0.5 * self.reg * np.sum(Wi*Wi)
    loss = data_loss + regularization_loss
